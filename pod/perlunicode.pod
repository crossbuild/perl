...
going forward (much higher on 64-bit systems).  If your code will ever
also run on an EBCDIC system, you should use a smaller maximum,
C<0x3FFF_FFFF>.  But, keep in mind that anything above C<0x10FFF> is
non-portable.
...
bytes to represent a given code point than UTF-8 does, The largest Unicode code points
take 5 bytes to represent (instead of 4 in UTF-8), and the largestPerl extends it
